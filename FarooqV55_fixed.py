...
# V51: Raised from 5.0 to 8.0 pips. V49 backtest showed BE at 5 was
# locking trades for tiny ~$8 wins. 8 pips is the middle ground —
# enough room for trades to develop, but locks profit before reversal.
BREAKEVEN_ENABLED = True           # set False to disable auto-breakeven
MOVE_TO_BREAKEVEN_AFTER_PIPS = 8.0
...
def detect_market_regime(df, fast_ema=20, slow_ema=50): # SMOOTH FIX: 9/21 → 20/50
    """Detect market regime using EMAs + Structure Confirmation.

    SMOOTH FIX: EMA 9/21 crossed on every 2-3 candle bounce, flipping the M5
    trend bullish/bearish mid-crash. That broke the strong_momentum 3-TF
    alignment (all_bear) during the June 5 crash so no SELL ever fired. EMA
    20/50 is steadier — it holds the trend through minor pullbacks so a real
    directional move keeps the alignment intact long enough to trigger.
    """
    try:
        # 20/50 EMAs: steady enough to survive 2-3 bar bounces
...
def create_entry_target(df: pd.DataFrame, lookahead: int = 12, detrend_span: int = 100) -> pd.Series:
    # ── DETREND FIX ──────────────────────────────────────────────────────────
    # Raw-price forward-window labels inherit gold's secular drift: in a bull
    # market move_up is larger simply from drift, so bearish setups get BUY
    # labels and the AI learns "everything bounces → BUY". Subtract a slow EMA
    # (the trend) and label on the RESIDUAL — now a bearish setup that merely
    # under-performs the trend gets a SELL label. This is the same fix applied
    # to gold_bot.py V22 that balanced its BUY/SELL output.
    ema   = df['close'].ewm(span=detrend_span, adjust=False).mean()
    r_close = df['close'] - ema
    r_high  = df['high']  - ema
    r_low   = df['low']   - ema
    ...
    # Forward extremes on the DETRENDED residual series
    future_max = r_high.shift(-1).rolling(indexer).max()
    future_min = r_low.shift(-1).rolling(indexer).min()

    move_up = future_max - r_close
    move_down = r_close - future_min

    # Symmetric barriers (impulse == max_pain) so neither side is favored.
    impulse = atr * 1.0
    max_pain = atr * 1.0

    valid = atr.notna() & future_max.notna() & future_min.notna()

    target = pd.Series(0, index=df.index)

    # 🟢 PURE BUY: Did it hit target without hitting the maximum pain?
    buy_cond = valid & (move_up > impulse) & (move_down < max_pain)
    
    # 🔴 PURE SELL: Did it hit target without hitting the maximum pain?
    sell_cond = valid & (move_down > impulse) & (move_up < max_pain)

    conflict = buy_cond & sell_cond

    target.loc[buy_cond & ~conflict] = 1
    target.loc[sell_cond & ~conflict] = -1

    target.iloc[-lookahead:] = 0
...
def train_models(df):
    ...
    try:
        ...
        # ── Label generation ──────────────────────────────────────────────────
        y_entry_all = create_entry_target(df, lookahead=12)
        ...
        w_entry   = weights_series[valid_mask].values
...
def calculate_liquidity_based_sl(direction, entry_price, df, fvgs, obs, atr=None):
    ...
    try:
        ...
        if direction.lower() in ["buy", "bullish"]:
            ...
            # 3. VOLATILITY CLAMP: Ensure SL is at least 0.6x ATR away
            # V51: Reduced from 0.8x → 0.6x ATR. V49 backtest showed 16
            # losses averaging -$23.71, much larger than wins. Tighter SL
            # caps loss size. Risk: more SL hits if 0.6x is too tight.
            min_dist = atr * 0.6
            ...
            # Volatility Clamp for Sells — V51: 0.8 → 0.6 ATR (matches BUY)
            min_dist = atr * 0.6
...
def calculate_liquidity_based_tp(direction, entry_price, df, atr=None):
    ...
    try:
        ...
        # 3. RR guard: TP must be at least 1.0x ATR away from entry
        # V51: Reduced from 1.5x → 1.0x ATR. V49 backtest showed only 2/54
        # trades reached TP (3.7%). Tighter TP fires more often. With SL
        # also tightened to 0.6 ATR, R:R becomes 1.67:1 (better than V49's 1:1).
        min_tp_dist = atr * 1.0
        if pool_dir == "buy":
...
def should_enter_trade(df, mtf_data, news_active):
    ...
    try:
        ...
        # (moved ABOVE weak_ai so strong_momentum can use the trends)
        # =============================
        h1_trend  = mtf_data.get("H1_trend")  if mtf_data else None
        m15_trend = mtf_data.get("M15_trend") if mtf_data else None
        m5_trend  = detect_market_regime(df)   # V55: EMA regime (matches backtest)

        # ═══════════════════════════════════════════════════════════════════
        # V55 NEW: STRONG MOMENTUM PATH (added on top of V52's full SMC chain)
        # ═══════════════════════════════════════════════════════════════════
        # WHY THIS EXISTS:
        #   On fast moves (e.g. the June 5 crash) V52's SMC features INVERT —
        #   net_bias/confluence read bullish while price crashes, because the
        #   FVG/OB structure lags. So V52 took ZERO trades on June 5 even though
        #   the AI correctly leaned SELL. This path bypasses the lagging features
        #   using raw, non-lagging price action (velocity + bias).
        #
        # SYMMETRIC by design (fixes the BUY-starvation bug): it only requires
        # the AI to LEAN the right way (buy_prob>sell_prob), NOT clear the high
        # 0.55 gate — because strong velocity + bias + M5 trend are already
        # strong evidence. Tested 3wk: 54tr 22B/32S, +$288, -2.5% DD.
        #
        # It is evaluated BEFORE the weak_ai filter so the muted-but-correct AI
        # during fast moves doesn't get blocked. The bias gate (both vel3 AND
        # bias12 must agree) prevents buying bounces / selling dips.
        # ═══════════════════════════════════════════════════════════════════
        SM_VEL  = 2.5    # min |3-bar velocity| in ATR units
        SM_BIAS = 1.5    # min |12-bar bias| in ATR units
        try:
            _atr = (df["high"] - df["low"]).tail(15).mean()
            if np.isfinite(_atr) and _atr > 0:
                _vel3   = (df["close"].iloc[-1] - df["close"].iloc[-4])  / _atr
                _bias12 = (df["close"].iloc[-1] - df["close"].iloc[-13]) / _atr

                # CHOP FILTER: require ALL THREE timeframes (M5+M15+H1) to agree
                # with the trade direction. In choppy markets the timeframes
                # disagree, which blocks strong_mom and avoids whipsaw losses.
                # Tested: turned March from -$126 to +$91, kept May +$108.
                _all_bull = (m5_trend == "bullish" and m15_trend == "bullish" and h1_trend == "bullish")
                _all_bear = (m5_trend == "bearish" and m15_trend == "bearish" and h1_trend == "bearish")

                # BUY: bullish thrust + bullish bias + ALL TF bullish + AI leans buy
                if (_all_bull and _vel3 > SM_VEL and _bias12 > SM_BIAS
                        and buy_prob > sell_prob):
                    print(f"📐 STRONG_MOM BUY: vel3={_vel3:+.2f} bias12={_bias12:+.2f} "
                          f"M5/M15/H1 all bull | AI B:{buy_prob:.2f} S:{sell_prob:.2f}")
                    return "BUY", "strong_momentum", ai_prob, ms_data, buy_prob, sell_prob

                # SELL: bearish thrust + bearish bias + ALL TF bearish + AI leans sell
                if (_all_bear and _vel3 < -SM_VEL and _bias12 < -SM_BIAS
                        and sell_prob > buy_prob):
                    print(f"📐 STRONG_MOM SELL: vel3={_vel3:+.2f} bias12={_bias12:+.2f} "
                          f"M5/M15/H1 all bear | AI B:{buy_prob:.2f} S:{sell_prob:.2f}")
                    return "SELL", "strong_momentum", ai_prob, ms_data, buy_prob, sell_prob
        except Exception as _e:
            pass  # if velocity calc fails, fall through to V52's normal chain
        ...
        # Aligned with ai_decision()'s existing BUY_THRESHOLD=0.55 / SELL_THRESHOLD=0.45.
        # =============================
        if buy_prob > sell_prob and buy_prob < 0.55:
            return "WAIT", "weak_ai", ai_prob, ms_data, buy_prob, sell_prob
        elif sell_prob > buy_prob and sell_prob < 0.45:
            return "WAIT", "weak_ai", ai_prob, ms_data, buy_prob, sell_prob
        ...
        if buy_prob > sell_prob:
            ...
            # ── CONFLUENCE ENTRY ───────────────────────────────────────
            # V52: AI threshold raised 0.55 → 0.70. V51 backtest showed
            # 26 trades with ai_prob 0.55-0.70 had net +$0.25 (noise).
            # 6 trades with ai_prob ≥ 0.70 had net +$17.16 (real signal).
            min_conf = 1.5 if ai_prob > 0.72 else 2.5
            if features.get("bullish_confluence", 0) >= min_conf and ai_prob > 0.70:
            ...
            # ── MTF ALIGNMENT (V52: threshold 0.72 → 0.99, effectively disabled)
            # V49 backtest: 22 trades, net -$48 | V51 backtest: 16 trades, net -$35
            # Two backtests, both losing. 0.99 makes it almost impossible to fire.
            # Soft-disable preserves code structure for future re-enable if needed.
            if m5_trend == "bullish" and m15_trend == "bullish" and ai_prob > 0.99:
        ...
        # =========================
            ...
            # ── CONFLUENCE ENTRY ───────────────────────────────────────
            # ── CONFLUENCE ENTRY (V52: threshold 0.55 → 0.70, mirror BUY)
            min_conf = 1.5 if ai_prob > 0.72 else 2.5
            if features.get("bearish_confluence", 0) >= min_conf and ai_prob > 0.70:
            ...
            # ── MTF ALIGNMENT ──────────────────────────────────────────
            # ── MTF ALIGNMENT (V52: 0.72 → 0.99, mirror BUY soft-disable)
            if m5_trend == "bearish" and m15_trend == "bearish" and ai_prob > 0.99:
