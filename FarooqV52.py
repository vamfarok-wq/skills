...
def detect_liquidity_sweep(df, lookback=20, min_sweep=0.2):
    ...
    try:
        ...
        # ATR for minimum sweep distance — 0.5×ATR filters noise spikes
        atr = (df['high'] - df['low']).rolling(14).mean().iloc[-1]
        min_dist = atr * 0.5
...
def create_entry_target(df: pd.DataFrame, lookahead: int = 12) -> pd.Series:
    ...
    future_max = df['high'].shift(-1).rolling(indexer).max()
    future_min = df['low'].shift(-1).rolling(indexer).min()

    move_up = future_max - df['close']
    move_down = df['close'] - future_min

    # Pure, mathematical targets:
    impulse = atr * 1.0
    max_pain = atr * 1.5
...
def should_enter_trade(df, mtf_data, news_active):
    ...
    try:
        ...
        # --- REGIME FILTER: block ranging, transitional, volatile, counter-trend ---
        try:
            from regime_detector import detect_regime, regime_allows_trade
            _tentative = "BUY" if buy_prob > sell_prob else "SELL"
            _regime    = detect_regime(df)
            _allowed, _regime_reason = regime_allows_trade(_regime, _tentative)
            if not _allowed:
                return "WAIT", f"regime_{_regime_reason}", ai_prob, ms_data, buy_prob, sell_prob
        except Exception:
            pass  # regime_detector not present — skip filter, don't block trading
        ...
        if buy_prob > sell_prob:
            ...
            # ── SMC REVERSAL — needs ai_prob > 0.68 to avoid noise sweeps ─
            if features.get("bullish_reversal_setup", 0) == 1 and ai_prob > 0.68:
            ...
            if m5_trend == "bullish" and m15_trend == "bullish" and ai_prob > 0.90:
        ...
        # =========================
            ...
            # ── SMC REVERSAL — needs ai_prob > 0.68 to avoid noise sweeps ─
            if features.get("bearish_reversal_setup", 0) == 1 and ai_prob > 0.68:
            ...
            if m5_trend == "bearish" and m15_trend == "bearish" and ai_prob > 0.90:
...
def execute_trade(direction, df, ai_prob, ms_data=None):
    ...
    try:
        ...
        # Enforce minimum SL distance of 1.5×ATR — prevents stops too tight for M5 noise
        sl_min_dist = atr * 1.5
        if direction == "buy":
            sl = min(sl, price - sl_min_dist)
        else:
            sl = max(sl, price + sl_min_dist)
