#!/usr/bin/env python3
"""V55 velocity-only path, trained on 20000 bars, tested May 29 – June 5."""
...
spec = importlib.util.spec_from_file_location("v55",
    '/root/.claude/uploads/b0a86e2b-be62-5eff-9440-c5581c2fb59d/f1f11239-FarooqV55.py')
...
train_end  = df[df['time'] < '2026-05-29'].index[-1] + 1   # 67534
test_start = train_end

print(f"Total bars available before May 29: {train_end}")
...
# ── MTF builder (EMA 9/21 for M5, 21/50 for M15, 50/99 for H1 — mirrors V55) ──
def build_mtf(ctx):
    c   = ctx['close']
    ...
    m5  = "bullish" if e9 >e21 else "bearish" if e9 <e21 else "range"
...
def atr14(s):
    return float((s['high'] - s['low']).rolling(14).mean().iloc[-1])

# ── Backtest ──────────────────────────────────────────────────────────────────
print("="*60)
print("  BACKTEST  May 29 – June 5, 2026  (velocity-only path)")
print("="*60)

trades = []; equity = EQUITY0; open_pos = []; last_entry = -999
skips  = {k: 0 for k in ['session','no_vel','sl_range']}
...
for idx in range(test_start + CONTEXT, len(df) - 1):
    ...
    h = bt.hour
    if not((7<=h<12)or(13<=h<21)): skips['session']+=1; continue

    # ── Velocity signal ───────────────────────────────────────────────────────
    mtf   = build_mtf(ctx)
    m5t   = v55.detect_market_regime(ctx)   # EMA 9/21 (V55 native)
    m15t  = mtf['M15_trend']
    h1t   = mtf['H1_trend']
    ...
    if   all_bull and vel3 >  SM_VEL and bias12 >  SM_BIAS and buy_p  > sell_p:
        ...
        skips['no_vel'] += 1; continue

    if any(p['dir'] == direction.lower() for p in open_pos): continue

    # ── SL / TP (V55: SL=0.6×ATR, TP=1.0×ATR, enforce MIN_RR=1.5) ───────────
    _atr = atr14(ctx)
    ...
    tp = entry_p + _atr*1.5 if d=='buy' else entry_p - _atr*1.5   # 1.5×ATR → RR=2.5

    sl_pips = abs(entry_p - sl) / GOLD_PIP
    if sl_pips < SL_MIN_PIPS or sl_pips > SL_MAX_PIPS:
        skips['sl_range'] += 1; continue
...
if not trades:
    ...
    print()
    print(f"  {'Time':<22} {'Dir':<5} {'Entry':>8} {'SL':>8} {'TP':>8} "
          f"{'vel3':>6} {'bias12':>7} {'buy_p':>6} {'sell_p':>7} {'Result':<8} {'P&L':>8}")
    print("  " + "-"*98)
    for _, r in tdf.iterrows():
        print(f"  {str(r.open_time):<22} {r.dir.upper():<5} {r.entry:>8.2f} "
              f"{r.sl:>8.2f} {r.tp:>8.2f} {r.vel3:>+6.2f} {r.bias12:>+7.2f} "
